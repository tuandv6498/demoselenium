            ...
            if (IsElementPresent(By.XPath("//span[contains(text(),'No deployment') or contains(text(),'デプロイなし')]")) == false)
            {
                ...
                if (String.Compare("stop", stateStoping, true) == 0 )
                {
                    if (String.Compare(curState, "stop", true) == 0 || String.Compare(curState, "停止", true) == 0)
                    {
                        ...
                        wait_90.Until(ExpectedConditions.ElementExists((By.XPath("//li[contains(@aria-label, 'New notification with title Successfully') or contains(@aria-label, 'クラウド サービスが正常に開始されました というタイトルの新しい通知。') or contains(@aria-label, 'クラウド サービスが正常に停止しました というタイトルの新しい通知。')]"))));
                        if (IsElementPresent(By.XPath("//li[contains(@aria-label, 'New notification with title Successfully') or contains(@aria-label, 'クラウド サービスが正常に開始されました というタイトルの新しい通知。') or contains(@aria-label, 'クラウド サービスが正常に停止しました というタイトルの新しい通知。')]")) == true)
                        {
                            countSuccess++;
                        }
                        Thread.Sleep(3000);
